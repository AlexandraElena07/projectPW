...
if(isset($_POST['add_to_cart'])){
    ...
    $select_cart = mysqli_query($conn, "SELECT * FROM `cart` WHERE title = '$book_name'");
 
    if(mysqli_num_rows($select_cart) > 0){
       $message[] = 'Book already added to cart';
    }else{
       $insert_product = mysqli_query($conn, "INSERT INTO `cart`(title, author, price, image, quantify) VALUES('$book_name', '$book_author', '$book_price', '$book_image', '$book_quantify')");
...
<div class="container">
    <section class="products">
        <h1 class="heading">Supplies/Accessories</h1>
        <div class="box-container">
        
        <?php
      
            $select_products = mysqli_query($conn, "SELECT * FROM `supplies`");
        ...
        <form action="" method="post">
         <div class="box">
            ...
            <h5><?php echo $fetch_product['gender_type']; ?></h5>
            <div class="price" style = "font-size: 20px"><?php echo $fetch_product['price']; ?>RON</div>
            <input type="hidden" name="book_name" value="<?php echo $fetch_product['name']; ?>">
